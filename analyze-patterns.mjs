...
function parseIntegerFlag(raw, flag, fallback, { min, label }) {
  if (raw === undefined) return fallback;
  const text = String(raw).trim();
  const value = /^\d+$/.test(text) ? Number(text) : NaN;
  if (!Number.isSafeInteger(value) || value < min) {
    console.error(`Error: ${flag} requires a ${label}: ${text}`);
    process.exit(1);
  }
  return value;
}

const MIN_THRESHOLD = parseIntegerFlag(flagValue(args, '--min-threshold'), '--min-threshold', 5, {
  min: 0,
  label: 'non-negative integer',
});

const MIN_VENDOR_N = parseIntegerFlag(flagValue(args, '--min-vendor-n'), '--min-vendor-n', 8, {
  min: 1,
