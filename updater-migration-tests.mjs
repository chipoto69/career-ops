...
const twoPassManifestChecks = [
  ...
  {
    // Force-add concrete file entries so a stale user .gitignore cannot abort
    // the update commit after checkout has already materialized the file.
    name: 'addPaths force-adds concrete file entries under literal pathspecs so stale user ignores cannot abort the update commit',
    pattern: /runGit\('--literal-pathspecs',\s*'add',\s*'-f',\s*'--',\s*\.\.\.batch\)/,
  },
  {
    // Force-add must never recurse into directories: ignored user files matched
    // by safety nets must not be swept into the system commit.
    name: 'addPaths rejects directory pathspecs before force-add can recurse into ignored user content',
    pattern: /rejectDirectories\(paths, ctx\.root \|\| ROOT\)/,
  },
  {
