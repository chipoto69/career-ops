...
1. Read `{{JD_FILE}}`. The batch runner pre-populates this file from the static prefetch and, when needed, the ATS API fetcher.
2. If the file is empty or missing, try to fetch the JD from `{{URL}}` with WebFetch.
3. If both sources fail, this is a hard stop — do ALL of the following, in this exact order, and nothing else:
