...
// which git treats as a no-op.
{
  ...
  // The promise this function makes is that it never rewrites a local line. The
  // protected directory negation remains a single local-owned line; only the new
  // upstream block is inserted before it so the negation still wins.
  eq(lines.filter((l) => l === '!test-fixtures/**').length, 1, 'the local negation is preserved once, not duplicated');
