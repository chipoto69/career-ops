...
export function roleFuzzyMatch(a, b) {
  ...
  if ((lvlA.size > 0) !== (lvlB.size > 0)) {
    ...
    const hasMeaningfulUniqueA = uniqueA.some(w => !BASELINE_TOKENS.has(w));
    const hasMeaningfulUniqueB = uniqueB.some(w => !BASELINE_TOKENS.has(w));
    if (uniqueA.length > 0 && uniqueB.length > 0 &&
        hasMeaningfulUniqueA && hasMeaningfulUniqueB) return false;
  }
